...
clistsum_read(void)
{
  ...
  while (1) {
	...
	if (head == NULL){ //if the head node is the first node
	head = newNode; //makes the head node point to newnode
	tmp = head; //makes head the temperary node
	}
	
	else{ //if head is not the first node
	tmp -> next = newNode; //makes tmp the next node
	tmp = newNode; //reassigns tmp to the last node in the list
	}	
...
void clistsum_free(void *head)
{
  ...
  while (hnode != NULL) { //continues looping until the head node is null
    // see man free and examples in C Book
    // force crash -- replace with your code here
    
	tmp = hnode; 
	hnode= hnode->next; //points to the next node
