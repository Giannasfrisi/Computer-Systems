...
cupper(long long cres, char *cptr)
{
  ...
  for (i=0; cptr[i] != 0; i++) { //uses a for loop to increment through cptr
    c = cptr[i];
    int length = ('a' - 'A'); //the difference in value between the upper and lower case letters
    if ((c >= 'a') && (c <= 'z')) { //if the letter is a lower case letter
	    c = c - length; //makes c the upper case verison
	    cptr[i] = c; //reassigns the letter 
