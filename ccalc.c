...
cmd2func(char c) {
  ...
  case 'S':
    // this smells fishy ;-)
   //  func = NULL;
    func = &csum; //sets func equal to the address of csum instead of null
    break;
  case 'U':
...
if (write(1, &result, sizeof(result)) != sizeof(result)) { //prints to result
    fprintf(stderr, "ERROR: write results failed\n");	
  }
  
if (write(1, &SUM_POSITIVE, sizeof(SUM_POSITIVE)) != sizeof(SUM_POSITIVE)) { //prints to sum_positive 
    fprintf(stderr, "ERROR: SUM_POSITIVE failed\n"); //uses the same format as the print to result
  }

if (write(1, &SUM_NEGATIVE, sizeof(SUM_NEGATIVE)) != sizeof(SUM_NEGATIVE)) { //prints to sum_negative
    fprintf(stderr, "ERROR: write SUM_NEGATIVE failed\n"); //also uses the same format
  }

   long long data_size = (void *)&CALC_DATA_END - (void *)&CALC_DATA_BEGIN; //prints to data size
    if (write(1,(void *)&CALC_DATA_BEGIN, data_size) != data_size) { //uses the void * type
	fprintf(stderr, "ERROR: write data size failed\n");
    }
